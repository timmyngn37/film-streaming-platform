...
describe('POST /api/films - validation', () => {
    it('should return 400 if releaseYear is invalid', async () => {
        const res = await request(app)
            .post('/api/films')
            .set('Authorization', `Bearer ${adminToken}`)
            .send({ title: 'Test Film', releaseYear: -1 });
        expect(res.statusCode).toBe(400);
        expect(res.body.error).toBe('releaseYear must be a positive integer');
    });

    it('should return 400 if releaseYear is not a number', async () => {
        const res = await request(app)
            .post('/api/films')
            .set('Authorization', `Bearer ${adminToken}`)
            .send({ title: 'Test Film', releaseYear: 'abc' });
        expect(res.statusCode).toBe(400);
    });
});

describe('PUT /api/films/:id - additional', () => {
    it('should return 400 for invalid id', async () => {
        const res = await request(app)
            .put('/api/films/invalidid')
            .set('Authorization', `Bearer ${adminToken}`)
            .send({ title: 'New Title' });
        expect(res.statusCode).toBe(400);
    });
    ...
    it('should update description and releaseYear', async () => {
        const film = await Film.create({ title: 'Test', releaseYear: 2020 });
        const res = await request(app)
            .put(`/api/films/${film._id}`)
            .set('Authorization', `Bearer ${adminToken}`)
            .send({ description: 'New desc', releaseYear: 2025 });
        expect(res.statusCode).toBe(200);
        expect(res.body.description).toBe('New desc');
        expect(res.body.releaseYear).toBe(2025);
    });
});

describe('DELETE /api/films/:id - additional', () => {
    it('should return 400 for invalid id', async () => {
        const res = await request(app)
            .delete('/api/films/invalidid')
            .set('Authorization', `Bearer ${adminToken}`);
        expect(res.statusCode).toBe(400);
    });
});
